<?php return array (
  ...
  array (
    ...
    array (
      ...
      'Alert' => 'Тревога',
      'All Days' => 'Все дни',
      'All listed domain names should point to this CloudmuCast installation. Separate multiple domain names with commas.' => 'Все перечисленные доменные имена должны указывать на эту установку CloudmuCast. Разделите несколько доменных имен запятыми.',
      ...
      'CloudmuCast First-Time Setup' => 'Первоначальная Настройка CloudmuCast',
      'CloudmuCast Instance Name' => 'Название этого экземпляра CloudmuCast',
      'CloudmuCast ships with a built-in free IP geolocation database. You may prefer to use the MaxMind GeoLite service instead to achieve more accurate results. Using MaxMind GeoLite requires a license key, but once the key is provided, we will automatically keep the database updated.' => 'CloudmuCast поставляется со встроенной бесплатной базой данных IP-геолокации. При желании, вместо неё вы можете использовать сервис геолокации MaxMind для достижения более точных результатов. Для использования MaxMind GeoLite требуется лицензионный ключ, как только ключ будет предоставлен, мы будем автоматически обновлять базу данных.',
      'CloudmuCast Update Checks' => 'Проверка обновлений CloudmuCast',
      'CloudmuCast User' => 'Пользователь CloudmuCast',
      'CloudmuCast uses a role-based access control system. Roles are given permissions to certain sections of the site, then users are assigned into those roles.' => 'CloudmuCast использует систему контроля доступа, основанную на ролях. Роли имеют права доступа к определенным разделам сайта, а затем пользователям назначаются эти роли.',
      'CloudmuCast Wiki' => 'CloudmuCast Wiki',
      'CloudmuCast will scan the uploaded file for matches in this station\'s music library. Media should already be uploaded before running this step. You can re-run this tool as many times as needed.' => 'CloudmuCast просканирует загруженный файл на совпадения в музыкальной библиотеке этой станции. Медиафайлы уже должны быть загружены перед выполнением этого шага. Вы можете повторно запускать этот инструмент столько раз, сколько необходимо.',
      'AzuraRelay is a standalone service that connects to your CloudmuCast instance, automatically relays your stations via its own server, then reports the listener details back to your main instance. This page shows all currently connected instances.' => 'AzuraRelay - это автономный сервис, который подключается к вашему экземпляру CloudmuCast, автоматически ретранслирует ваши станции через свой собственный сервер, а затем передает сведения о слушателе обратно в ваш основной экземпляр. На этой странице показаны все подключенные в данный момент экземпляры.',
      'Back' => 'Назад',
      ...
      'Customize CloudmuCast Settings' => 'Настройка Параметров CloudmuCast',
      ...
      'Donate to support CloudmuCast!' => 'Пожертвуйте на поддержку CloudmuCast!',
      'Down' => 'Вниз',
      'Download' => 'Скачать',
      ...
      'Enable to have CloudmuCast automatically run nightly backups at the time specified.' => 'Включите, чтобы CloudmuCast автоматически запускал ночное резервное копирование в указанное время.',
      ...
      'Enter "CloudmuCast" as the application name. You can leave the URL fields unchanged. For "Scopes", only "write:media" and "write:statuses" are required.' => 'Введите «CloudmuCast» в качестве имени приложения. Вы можете оставить поля URL без изменений. Для «Scopes» требуются только «write:media» и «write:statuses».',
      ...
      'Hide CloudmuCast Branding on Public Pages' => 'Скрыть бренд CloudmuCast на публичных страницах',
      ...
      'If a visitor is not signed in and visits the CloudmuCast homepage, you can automatically redirect them to the URL specified here. Leave blank to redirect them to the login screen by default.' => 'Если посетитель не авторизован и посещает главную страницу CloudmuCast, вы можете автоматически перенаправить его на указанный здесь URL. Оставьте пустым, чтобы перенаправить его на страницу входа по умолчанию.',
      ...
      'If enabled, CloudmuCast will automatically record any live broadcasts made to this station to per-broadcast recordings.' => 'Если эта функция включена, CloudmuCast будет автоматически записывать все трансляции прямого эфира, сделанные на этой станции, при каждом вещании.',
      'If enabled, CloudmuCast will connect to the MusicBrainz database to attempt to find an ISRC for any files where one is missing. Disabling this may improve performance.' => 'Если этот параметр включен, CloudmuCast будет подключаться к базе данных MusicBrainz, чтобы попытаться найти ISRC для любых файлов, где он отсутствует. Отключение этой опции может повысить производительность.',
      ...
      'If selected, this will remove the CloudmuCast branding from public-facing pages.' => 'Если выбрано, то будет удалён бренд CloudmuCast с публично расположенных страниц.',
      ...
      'Learn more about release channels in the CloudmuCast docs.' => 'Узнайте больше о каналах релизов в документации CloudmuCast.',
      ...
      'Note: This should be the public-facing homepage of the radio station, not the CloudmuCast URL. It will be included in broadcast details.' => 'Примечание. Это должна быть общедоступная домашняя страница радиостанции, а не URL-адрес CloudmuCast. Будет включено в подробности вещания.',
      'Notes' => 'Заметки',
      'Notice' => 'Уведомление',
      'Now' => 'Сейчас',
      ...
      'Powered by CloudmuCast' => 'Работает на CloudmuCast',
      ...
      'Settings' => 'Настройки',
      'Setup instructions for broadcasting software are available on the CloudmuCast wiki.' => 'Инструкции по настройке программного обеспечения для вещания доступны на CloudmuCast Wiki.',
      'SFTP Host' => 'SFTP хост',
      ...
      'Shoutcast 2 DNAS is not free software, and its restrictive license does not allow CloudmuCast to distribute the Shoutcast binary.' => 'Shoutcast 2 DNAS не является свободным программным обеспечением, и его ограничительная лицензия не позволяет CloudmuCast распространять бинарный файл Shoutcast.',
      ...
      'Show new releases within your update channel on the CloudmuCast homepage.' => 'Показывать новые релизы в вашем канале обновлений на главной странице CloudmuCast.',
      ...
      'Stereo Tool is not free software, and its restrictive license does not allow CloudmuCast to distribute the Stereo Tool binary.' => 'Stereo Tool не является бесплатным программным обеспечением, и его ограничительная лицензия не позволяет CloudmuCast распространять двоичный файл Stereo Tool.',
      ...
      'The editable text boxes are areas where you can insert custom configuration code. The non-editable sections are automatically generated by CloudmuCast.' => 'Редактируемые текстовые поля - это области, где вы можете вставить пользовательский код конфигурации. Не редактируемые разделы автоматически генерируются CloudmuCast.',
      ...
      'This is an advanced feature and custom code is not officially supported by CloudmuCast. You may break your station by adding custom code, but removing it should fix any issues.' => 'Пользовательский код в расширенных настройках официально не поддерживается CloudmuCast. Вы можете сломать свою станцию, добавив пользовательский код, но его удаление должно решить все проблемы.',
      ...
      'This mode disables CloudmuCast\'s AutoDJ management, using Liquidsoap itself to manage song playback. "next song" and some other features will not be available.' => 'Этот режим отключает АвтоДиджея CloudmuCast и для управления воспроизведением песен используется сам Liquidsoap. "Следующая песня" и некоторые другие функции будут не доступны.',
      ...
      'This name will appear as a sub-header next to the CloudmuCast logo, to help identify this server.' => 'Это название будет отображаться в качестве подзаголовка рядом с логотипом CloudmuCast, чтобы помочь идентифицировать этот сервер.',
      ...
      'This queue contains the remaining tracks in the order they will be queued by the CloudmuCast AutoDJ (if the tracks are eligible to be played).' => 'Эта очередь содержит треки в том порядке, в котором они будут поставлены в очередь АвтоДиджея CloudmuCast (если эти треки имеют право на воспроизведение).',
      ...
      'Update' => 'Изменение',
      'Update CloudmuCast' => 'Обновление CloudmuCast',
      'Update CloudmuCast via Web' => 'Обновление CloudmuCast через веб-браузер',
      'Update CloudmuCast? Your installation will restart.' => 'Обновить CloudmuCast? Ваша установка будет перезапущена.',
      ...
      'Use API keys to authenticate with the CloudmuCast API using the same permissions as your user account.' => 'Используйте ключи API для аутентификации с API CloudmuCast, используя те же разрешения, что и ваша учетная запись пользователя.',
      ...
      'Website' => 'Веб-сайт',
      'Wednesday' => 'Среда',
      'Welcome to CloudmuCast!' => 'Добро пожаловать в CloudmuCast!',
      ...
      'You can set a custom URL for this stream that CloudmuCast will use when referring to it. Leave empty to use the default value.' => 'Вы можете задать пользовательский URL для этого потока, который CloudmuCast будет использовать в ссылке на него. Оставьте пустым, чтобы использовать значение по умолчанию.',
      ...
      'CloudmuCast Application Log' => 'CloudmuCast - Журнал',
      'CloudmuCast Now Playing Log' => 'Журнал воспроизводимого сейчас CloudmuCast',
      'CloudmuCast Synchronized Task Log' => 'Журнал синхронизированных задач CloudmuCast',
      'CloudmuCast Queue Worker Log' => 'Журнал очереди рабочих процессов CloudmuCast',
      ...
      'This is a test message from CloudmuCast. If you are receiving this message, it means your e-mail settings are configured correctly.' => 'Это тестовое сообщение от CloudmuCast. Если вы получили это сообщение, это означает, что ваши настройки электронной почты настроены правильно.',
      ...
      'Set Up CloudmuCast' => 'Настройка CloudmuCast',
      ...
      'Select...' => 'Выбрать...',
      'CloudmuCast Installer' => 'Установщик CloudmuCast',
      'Welcome to CloudmuCast! Complete the initial server setup by answering a few questions.' => 'Добро пожаловать в CloudmuCast! Завершите первоначальную настройку сервера, ответив на несколько вопросов.',
      'CloudmuCast Updater' => 'Обновление CloudmuCast',
      'Change installation settings?' => 'Изменить параметры установки?',
      'CloudmuCast is currently configured to listen on the following ports:' => 'В данный момент CloudmuCast настроен для прослушивания следующих портов:',
      'HTTP Port: %d' => 'HTTP порт: %d',
      'HTTPS Port: %d' => 'HTTPS порт: %d',
      'SFTP Port: %d' => 'SFTP порт: %d',
      'Radio Ports: %s' => 'Радио порты: %s',
      'Customize ports used for CloudmuCast?' => 'Настроить порты, используемые для CloudmuCast?',
      'Writing configuration files...' => 'Запись файлов конфигурации...',
      'Server configuration complete!' => 'Настройка сервера завершена!',
      'This file was automatically generated by CloudmuCast.' => 'Этот файл был автоматически создан CloudmuCast.',
      ...
      'HTTP Port' => 'HTTP порт',
      'The main port CloudmuCast listens to for insecure HTTP connections.' => 'Основной порт CloudmuCast прослушивает небезопасные HTTP соединения.',
      'HTTPS Port' => 'HTTPS порт',
      'The main port CloudmuCast listens to for secure HTTPS connections.' => 'Основной порт CloudmuCast прослушивает безопасные HTTPS соединения.',
      'The port CloudmuCast listens to for SFTP file management connections.' => 'Порт CloudmuCast прослушивает соединения для управления файлами SFTP.',
      'Station Ports' => 'Порты Станции',
      'The ports CloudmuCast should listen to for station broadcasts and incoming DJ connections.' => 'Порты, которые CloudmuCast должен прослушивать для вещания станций и входящих подключений Диджеев.',
      ...
      'CloudmuCast Initializing...' => 'Инициализация CloudmuCast...',
      'CloudmuCast Setup' => 'Настройка CloudmuCast',
      'Welcome to CloudmuCast. Please wait while some key dependencies of CloudmuCast are set up...' => 'Добро пожаловать в CloudmuCast. Пожалуйста, подождите, пока настраиваются некоторые ключевые зависимости CloudmuCast...',
      ...
      'CloudmuCast is now updated to the latest version!' => 'CloudmuCast теперь обновлен до последней версии!',
      'CloudmuCast installation complete!' => 'Установка CloudmuCast завершена!',
      ...
      'New CloudmuCast Stable Release Available' => 'Доступен новый стабильный релиз CloudmuCast',
      ...
      'New CloudmuCast Rolling Release Available' => 'Доступен новый Rolling Release CloudmuCast',
      ...
      'You can track the execution time and memory usage of any CloudmuCast page or application from the profiler page.' => 'Вы можете отслеживать время выполнения и использование памяти любой страницы или приложения CloudmuCast со страницы профилирования.',
      ...
      'If you regularly use different URLs to access CloudmuCast, you should enable the "Prefer Browser URL" setting.' => 'Если вы регулярно используете разные URL-адреса для доступа к CloudmuCast, вам следует включить параметр "Предпочтительно URL-адрес в браузере".',
      ...
      'This likely means that multiple CloudmuCast instances are connecting to the same database. This instance is not the current active one, so synchronized tasks won\'t run on this server. If this is intentional, you can disregard this message.' => 'Вероятно, это означает, что несколько экземпляров CloudmuCast подключаются к одной и той же базе данных. Этот экземпляр не является текущим активным, поэтому синхронизированные задачи не будут выполняться на этом сервере. Если это сделано намеренно, вы можете проигнорировать это сообщение.',
