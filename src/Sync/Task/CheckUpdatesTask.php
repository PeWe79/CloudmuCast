...
final class CheckUpdatesTask extends AbstractTask
{
    ...
    public function run(bool $force = false): void
    {
        ...
        if ($this->environment->isTesting()) {
            $this->logger->info('Update checks are currently disabled for this CloudmuCast instance.');
            return;
        }
        ...
        try {
            ...
            if (!empty($updates)) {
                ...
                $this->logger->error('Error parsing update data response from CloudmuCast central.');
            }
        } catch (TransferException $e) {
            $this->logger->error(sprintf('Error from CloudmuCast Central (%d): %s', $e->getCode(), $e->getMessage()));
            return;
        }
