...
setup-release() {
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/main/sample.env -o .env
  fi
...
check-install-requirements() {
  ...
  echo "Checking installation requirements for CloudmuCast..."
  ...
  if [[ $CURRENT_OS == "Linux" ]]; then
    ...
    echo "       Automated CloudmuCast installation is not currently supported on this"
  ...
  if [[ $SCRIPT_DIR == "/var/azuracast" ]]; then
    ...
    echo "       CloudmuCast is not installed in /var/azuracast, as is recommended"
    echo "       for most installations. This will not prevent CloudmuCast from"
...
run-installer() {
  ...
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/sample.env -o .env
  fi
  if [[ ! -f azuracast.env ]]; then
    curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/azuracast.sample.env -o azuracast.env
  fi
  if [[ ! -f docker-compose.yml ]]; then
    curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.yml
  fi
  ...
  curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/docker-compose.installer.yml -o docker-compose.installer.yml
...
#
# Run the initial installer of Docker and CloudmuCast.
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/docker-compose.sample.yml -o docker-compose.new.yml
    fi
...
update-self() {
  ...
  curl -H 'Cache-Control: no-cache, no-store' -fsSL \
    https://raw.githubusercontent.com/PeWe79/CloudmuCast/$AZURACAST_RELEASE_BRANCH/docker.sh?$(date +%s) \
    -o docker.sh
  chmod a+x docker.sh
